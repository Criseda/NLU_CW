"""
train_electra_continue.py — Continue training ELECTRA from checkpoint until early stopping.

Usage:
    python -m src.solution2.electra_model.train_electra_continue
"""
...
from . import config_continue as config
...
def main():
    """Main training loop for continued training."""
    ...
    # ── Load checkpoint ────────────────────────────────────────────────────────
    print(f"[train] Loading checkpoint from {config.CHECKPOINT_PATH}")
    if not os.path.exists(config.CHECKPOINT_PATH):
        raise FileNotFoundError(f"Checkpoint not found: {config.CHECKPOINT_PATH}")
    
    model = load_model(config.MODEL_NAME, checkpoint_path=config.CHECKPOINT_PATH, token=config.HF_TOKEN).to(device)
    print(f"[train] Model loaded with checkpoint")
    ...
    # ── Evaluate checkpoint on dev set first ────────────────────────────────────
    print("\n[train] Evaluating checkpoint before continuing...")
    dev_metrics = evaluate(model, dev_loader, device)
    print(
        f"[eval]  F1: {dev_metrics['f1']:.4f} | "
        f"Acc: {dev_metrics['acc']:.4f} | "
        f"Precision: {dev_metrics['precision']:.4f} | "
        f"Recall: {dev_metrics['recall']:.4f} (checkpoint)"
    )
    best_f1 = dev_metrics["f1"]
    
    # Training loop
    best_checkpoint = config.CHECKPOINT_PATH
    patience_counter = 0
