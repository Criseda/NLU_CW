...
def train(checkpoint_path: str | None = None) -> None:
    ...
    # ── Model ──────────────────────────────────────────────────────────────────
    model = load_model(model_name=config.MODEL_NAME, checkpoint_path=checkpoint_path, token=config.HF_TOKEN).to(device)
    ...
    patience = 0
    early_stopping_patience = getattr(config, 'EARLY_STOPPING_PATIENCE', 3)  # Default to 3 if not in config
